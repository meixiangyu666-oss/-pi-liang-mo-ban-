...
def generate_header_for_sbv_brand_store(uploaded_bytes, sheet_name='广告模版'):
    ...
    with st.expander("查看详细日志", expanded=False):
        ...
        # ======== 【修改 1 最终版：精准锁定“广告活动名称”列】Start ========
        
        # 1. 定义哪些主题必须依赖全局设置
        strict_themes = ['SB落地页：商品集', 'SBV落地页：品牌旗舰店']
        
        # 2. 预扫描：精准检查这些主题下的“广告活动名称”列是否有值
        ...
        for theme in strict_themes:
            ...
            if h_row is not None and e_row > h_row:
                ...
                # B. 如果找到了“广告活动名称”这一列，就检查这一列下面有没有数据
                if target_col_idx != -1:
                    # 提取这一列的数据 (从 header行+1 到 结束行)
                    col_data = df_survey.iloc[h_row + 1 : e_row + 1, target_col_idx]
                    
                    # 检查：转为字符串 -> 去空 -> 是否有任意非空值
                    if col_data.astype(str).str.strip().ne('').any():
                        has_strict_activities = True
                        st.write(f"🔍 触发全局检查: 在 '{theme}' 区域检测到有效的'广告活动名称'")
                        break # 只要发现一个实锤的活动，就必须检查全局，跳出循环
                else:
                    st.write(f"ℹ️ 在 '{theme}' 区域未找到'广告活动名称'列，跳过检测")
        ...
        if has_strict_activities:
            ...
            if missing_globals:
                ...
                error_area.error(f"原因：检测到 '{theme}' 区域的“广告活动名称”列有值，说明您想创建此类广告。")
                error_area.error(f"此类广告强制要求在全局设置中填写：{missing_globals}")
                error_area.warning("⚠️ 程序已终止，请完善信息后重新上传。")
                os.unlink(input_file)
                return None
        else:
            st.info("ℹ️ 未检测到需全局设置的活动（'广告活动名称'列为空），跳过必填检查。")
            
        # ======== 【修改 1 最终版：精准锁定“广告活动名称”列】End ========
